...
plain — identity (tag/name/agent key) and a configured model only, with no
roles, operational modes, specialized prompts, or behavioral wrappers.
...
(`scripts/web_ui/settings.py`). The control plane (agents, models, fallback)
lives in `opencode.json`; the specs under `scripts/core/agents/` mirror it.
