...
def build_help_text() -> str:
    ...
    return (
        ...
        "                   GENERAL section = global execution mode (master-level\n"
        "                   model/mode defaults); AGENTS section = one row per\n"
        "                   registry agent (M1..M7) opening its own model/mode\n"
        "                   submenu — every agent is configurable individually;\n"
        ...
        "  /plan [prompt]   Analyzer Core: preview the mandatory pre-dispatch\n"
        "                   master plan (requirements -> modular decoupling ->\n"
        "                   one component per agent); no arg analyzes the input\n"
        ...
        "                   Structural text first gets an Analyzer master plan whose\n"
        "                   module map (one component per agent) is also persisted\n"
